...
* `+Auto Close Tag+` Useful for HTML, but maybe be disabled since
...
** `+Peacock+` Color my workspaces. Use `+Ctrl-Shift-p+` then
...
** `+Indent one space+` `+Space+` to indent, `+Shift+Space+` to decrease
indent, great!
...
** `+Todo++` Custom symbols. Toggle with `+Todo: cmd+`, didn't work for
me.
** `+Org Mode+`
